...
IBLIrradianceApp::~IBLIrradianceApp() {}

void IBLIrradianceApp::initShapes() {
    Cube cube{};
...
bool IBLIrradianceApp::load(std::shared_ptr<rhi::IRenderer> rhiRenderer) {
...
void IBLIrradianceApp::initCaptureViews() {
...
void IBLIrradianceApp::initFramebuffer() {
...
void IBLIrradianceApp::compileShader(const rhi::VertexLayout& cubeLayout) {
...
void IBLIrradianceApp::loadTexture() {
...
void IBLIrradianceApp::createIrradianceMap() {
    rhi::TextureDesc desc;
    desc.format = rhi::TextureFormat::RGB16F;
    desc.wrapS = desc.wrapT = desc.wrapR = rhi::TextureWrap::ClampToEdge;
    desc.minFilter = rhi::TextureFilter::Linear;
...
void IBLIrradianceApp::renderToCubemap() {
...
void IBLIrradianceApp::renderIrradianceMap() {
    const auto captureProjection = glm::perspective(glm::radians(90.0f), 1.0f, 0.1f, 10.0f);
    const auto captureViews = GetCaptureViews();
...
void IBLIrradianceApp::renderCube(const std::shared_ptr<rhi::IPipeline>& program, const glm::mat4& model) {
...
void IBLIrradianceApp::renderSphere(const std::shared_ptr<rhi::IPipeline>& program, const glm::mat4& model) {
    renderer()->setPipeline(program);
...
void IBLIrradianceApp::renderBeforeLoop() {
    renderToCubemap();
    renderer()->flush();
...
void IBLIrradianceApp::renderBackground(const std::shared_ptr<rhi::IPipeline>& program, const glm::mat4& view, const glm::mat4& projection) {
    renderer()->setPipeline(program);
    renderer()->bindTexture(m_envCubemap, 0);
...
void IBLIrradianceApp::renderObjectsAndLights(const std::shared_ptr<rhi::IPipeline>& program, const glm::mat4& view, const glm::mat4& projection) {
...
void IBLIrradianceApp::draw(const float dt) {

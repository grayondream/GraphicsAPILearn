...
std::shared_ptr<rhi::IPipeline> FrameBufferApp::compileShader(const std::string& name, const rhi::VertexLayout& layout) {
...
void FrameBufferApp::compileShader(const rhi::VertexLayout& layout) {
	_contentPipeline = compileShader("Basic", layout);
	_screenPipeline = compileShader("Screen", layout);
}

void FrameBufferApp::loadTexture() {
	{
...
void FrameBufferApp::initGLEnv() {
}

bool FrameBufferApp::load(std::shared_ptr<rhi::IRenderer> rhiRenderer) {
...
void FrameBufferApp::createFrameBuffer() {

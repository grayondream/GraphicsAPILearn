...
void SSAOApp::createFrameBuffers() {
	createGBufferFbo();
	createSSAOFbo();
}

void SSAOApp::createGBufferFbo() {
	const int w = static_cast<int>(windowWidth());
	const int h = static_cast<int>(windowHeight());
...
bool SSAOApp::load(std::shared_ptr<rhi::IRenderer> rhiRenderer) {
...
void SSAOApp::createTextures() {

}

void SSAOApp::compileShader(const rhi::VertexLayout& cubeLayout, const rhi::VertexLayout& quadLayout) {
...
void SSAOApp::initModelPipeline() {
	const auto shaderDir = join(StaticCollector::getGLShaderPath(), "Light", "Advanced", "SSAO");
...
void SSAOApp::renderOneCube() {
	renderer()->setVertexBuffer(m_cubeVb);
	renderer()->draw(m_cubeVertexCount, 0);
}

void SSAOApp::renderGBuffer(std::shared_ptr<rhi::IPipeline>& program, const glm::mat4& projection, const glm::mat4& view) {
...
void SSAOApp::renderSSAOTexture(std::shared_ptr<rhi::IPipeline>& program, const glm::mat4& projection){
	renderer()->setPipeline(program);
...
void SSAOApp::renderBlurSSAOTexture(std::shared_ptr<rhi::IPipeline>& program){
	renderer()->setPipeline(program);
...
void SSAOApp::renderLightPass(std::shared_ptr<rhi::IPipeline>& program){
	renderer()->setPipeline(program);
	renderer()->bindTexture(m_gBuffer.gPosition, 0);
	renderer()->bindTexture(m_gBuffer.gNormal, 1);
...
void SSAOApp::draw(const float dt) {

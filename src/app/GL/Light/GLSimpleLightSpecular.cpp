...
bool GLSimpleLightSpecular::init(const HINSTANCE inst, const WindowDesc& param) {
	...
	{
		const auto vfile = shaderDir / "specular" / "light.vert";
		const auto ffile = shaderDir / "specular" / "light.frag";
		auto ret = _lightProgram.init(vfile.string(), ffile.string());
		ErrorHandle::ExitIfFailed(ret, "Create OpenGL program failed!");
	}

	{
		const auto vfile = shaderDir / "specular" / "object.vert";
		const auto ffile = shaderDir / "specular" / "object.frag";
...
void GLSimpleLightSpecular::createVertexBuffer() {
...
void GLSimpleLightSpecular::clearColor() {
	return GLApp::clearColor();
}

void GLSimpleLightSpecular::beginDrawScene() {
...
void GLSimpleLightSpecular::endDrawScene() {
	return GLApp::endDrawScene();
}

void GLSimpleLightSpecular::onKeyBoardEvent(const UINT msg, const WPARAM wParam, const LPARAM lParam) {
	switch (msg) {
	case WM_KEYDOWN:
		break;
	case WM_KEYUP:
...
void GLSimpleLightSpecular::onMouseDown(const UINT msg, WPARAM btnState, int x, int y) {
	switch (msg) {
	case WM_LBUTTONDOWN:
...
void GLSimpleLightSpecular::onMouseUp(const UINT msg, WPARAM btnState, int x, int y) {
	switch (msg) {
	case WM_LBUTTONUP:
...
void GLSimpleLightSpecular::onMouseMove(WPARAM btnState, int x, int y) {
	if (!_mouseClicked) {
...
void GLSimpleLightSpecular::onMouseScroll(const UINT msg, const WPARAM wParam, const LPARAM lParam) {

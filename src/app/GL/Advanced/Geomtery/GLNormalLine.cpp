...
bool GLNormalLine::init(const HINSTANCE inst, const WindowDesc& param) {
...
void GLNormalLine::createVertexBuffer() {
...
void GLNormalLine::clearColor() {
	return GLApp::clearColor();
}

void GLNormalLine::beginDrawScene() {
...
void GLNormalLine::drawScene(const float dt) {
	...
	{
		_normalProgram.use();
		_normalProgram.update("projection", projection);
		_normalProgram.update("view", view);
		_normalProgram.update("model", model);
		glDrawElements(GL_TRIANGLES, shape.idxSize(), GL_UNSIGNED_INT, 0);
	}
	glBindVertexArray(0);
...
void GLNormalLine::endDrawScene() {
	return GLApp::endDrawScene();
}

void GLNormalLine::onKeyBoardEvent(const UINT msg, const WPARAM wParam, const LPARAM lParam) {
	switch (msg) {
	case WM_KEYDOWN:
		break;
	case WM_KEYUP:
...
void GLNormalLine::onMouseDown(const UINT msg, WPARAM btnState, int x, int y) {
	switch (msg) {
	case WM_LBUTTONDOWN:
...
void GLNormalLine::onMouseUp(const UINT msg, WPARAM btnState, int x, int y) {
	switch (msg) {
	case WM_LBUTTONUP:
...
void GLNormalLine::onMouseMove(WPARAM btnState, int x, int y) {
	if (!_mouseClicked) {
...
void GLNormalLine::onMouseScroll(const UINT msg, const WPARAM wParam, const LPARAM lParam) {

...
void main(){
    ...
    float theta = dot(lightDir, normalize(-lights[0].direction.xyz));
    float epsilon = lights[0].params.w - lights[0].direction.w;
    float intensity = clamp((theta - lights[0].direction.w) / epsilon, 0.0, 1.0);

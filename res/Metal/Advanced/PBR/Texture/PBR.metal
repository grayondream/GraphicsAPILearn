...
fragment float4 PBR_Texture_fragment(VertexOut in [[stage_in]],
    ...
    float3 albedo = albedoMap.sample(smp, in.TexCoords).rgb;
    float metallic = metallicMap.sample(smp, in.TexCoords).r;
    float roughness = roughnessMap.sample(smp, in.TexCoords).r;

...
vertex VertexOut IBL_Specular_PBR_vertex(VertexIn in [[stage_in]],
...
fragment float4 IBL_Specular_PBR_fragment(VertexOut in [[stage_in]],
                                          constant UniformBlock& ubo [[buffer(8)]],
    ...
    float3 F = F0 + (max(float3(1.0 - roughness), F0) - F0) * pow(clamp(1.0 - max(dot(N, V), 0.0), 0.0, 1.0), 5.0);
    
    float3 kS2 = F;
    float3 kD2 = 1.0 - kS2;
    kD2 *= 1.0 - metallic;
    
    float3 irradiance = irradianceMap.sample(smp, N).rgb;
    float3 diffuse = irradiance * albedo;
    
    const float MAX_REFLECTION_LOD = 4.0;
    float3 prefilteredColor = prefilterMap.sample(smp, R, level(roughness * MAX_REFLECTION_LOD)).rgb;
    float2 brdf = brdfLUT.sample(smp, float2(max(dot(N, V), 0.0), roughness)).rg;
    float3 specular2 = prefilteredColor * (F * brdf.x + brdf.y);
    
    float3 ambient = (kD2 * diffuse + specular2) * ao;

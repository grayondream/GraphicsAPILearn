...
vertex VertexOut Hdr_Hdr_vertex(VertexIn in [[stage_in]],
...
fragment float4 Hdr_Hdr_fragment(VertexOut in [[stage_in]],
                                 constant UniformBlock& ubo [[buffer(8)]],
                                 texture2d<float> hdrBuffer [[texture(0)]],
                                 sampler hdrSampler [[sampler(0)]]) {
    const float gamma = 2.2;
    float3 hdrColor = hdrBuffer.sample(hdrSampler, in.TexCoords).rgb;
    if(ubo.floatPool[13] > 0.5)   // hdr
    {
        // reinhard
